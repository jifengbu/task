...
export default async ({
    ...
    .group({
        _id: null,
        mf: { $sum: { $cond: { if: { $and: [{ $gte: ['$examineFinishTime', startMonth] }, { $eq: ['$state', 128] }] }, then: 1, else: 0 } } },
        muf: { $sum: { $cond: { if: { $ne: ['$state', 128] }, then: 1, else: 0 } } },
        qf: { $sum: { $cond: { if: { $and: [{ $gte: ['$examineFinishTime', startQuarter] }, { $eq: ['$state', 128] }] }, then: 1, else: 0 } } },
        quf: { $sum: { $cond: { if: { $ne: ['$state', 128] }, then: 1, else: 0 } } },
        yf: { $sum: { $cond: { if: { $and: [{ $gte: ['$examineFinishTime', startYear] }, { $eq: ['$state', 128] }] }, then: 1, else: 0 } } },
        yuf: { $sum: { $cond: { if: { $ne: ['$state', 128] }, then: 1, else: 0 } } },
    });
    ...
    return { success: true, context: {
        ...
        finishTask: {
            month: [finishTaskInfo.mf, finishTaskInfo.muf],
            quarter:  [finishTaskInfo.qf, finishTaskInfo.quf],
            year:  [finishTaskInfo.yf, finishTaskInfo.yuf],
        },
    } };
};
